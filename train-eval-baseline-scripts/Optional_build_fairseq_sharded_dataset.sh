...
# ------------------------ 1. Train your SPM model (or use the spm model we provided) ------------------------ #

# Put the SPM model like ${SPM_DATA_DIR}/spm_64k.model 


# ------------------------ 2. Encode all data using SPM model ------------------------ #
SPM_DATA_DIR= ... # fill in with your SPM Data Dir
DATASET_DIR= ... # fill in with your plain-dataset directory
...
for lang in bg so ca da be bs mt es uk am hi ro no ti de cs lb pt nl mr is ne ur oc ast ha sv kab gu ar fr ru it pl sr sd he af kn bn; do
    spm_encode --model ${SPM_DATA_DIR}/spm_64k.model < ${DATASET_DIR}/en-${lang}.en > ${SPM_DATA_DIR}/train.en-${lang}.en
    spm_encode --model ${SPM_DATA_DIR}/spm_64k.model < ${DATASET_DIR}/en-${lang}.${lang} > ${SPM_DATA_DIR}/train.en-${lang}.${lang}
    
done

# ------------------------ 3. Build Sharded dataset using SPM data ------------------------ #
